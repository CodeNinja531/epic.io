document.addEventListener('DOMContentLoaded', () => {
            ...
            // Define a constant for the fixed header's height offset
            ...
            randomSongBtn.addEventListener('click', () => {
                if (allSongElements.length > 0) {
                    ...
                    // Manual scroll for random button too
                    const scrollPosition = randomSongDiv.offsetTop - FIXED_HEADER_OFFSET;
                    window.scrollTo({
                    ...
                    if (parentSagaDiv) {
                        ...
                        updateBodyBackground(sagaName); // Update background for random song's saga
                    ...
                    toggleMenu(); // Closes menu for random song
                }
            });
            ...
            function displayMusic(musicalData) {
                ...
                musicalData.sagas.forEach((saga, sagaIndex) => {
                    ...
                    // Updated: Saga link click handler
                    navSagaLink.addEventListener('click', (e) => {
                        e.preventDefault();
                        const targetId = e.target.closest('a').getAttribute('href').substring(1);
                        const targetElement = document.getElementById(targetId);
                        if (targetElement) {
                            const scrollPosition = targetElement.offsetTop - FIXED_HEADER_OFFSET;
                            window.scrollTo({
                                top: scrollPosition > 0 ? scrollPosition : 0,
                                behavior: 'smooth'
                            });
                            // Update background immediately for the clicked saga
                        ...
                        // No toggleMenu() here, so the menu stays open for saga selection
                        navList.querySelectorAll('a').forEach(link => link.classList.remove('active'));
                        musicContainer.querySelectorAll('.song').forEach(song => song.classList.remove('active'));
                    ...
                    saga.songs.forEach((song, songIndex) => {
                        ...
                        // Updated: Song link click handler
                        navSongLink.addEventListener('click', (e) => {
                            ...
                            toggleMenu(); // Closes menu for song clicks
                        });
                    });
                ...
                const handleScroll = () => {
                    ...
                    // Iterate songs from bottom up to find the topmost one actively in view
                    for (let i = songs.length - 1; i >= 0; i--) {
                        const song = songs[i];
                        const rect = song.getBoundingClientRect();
                        // Check if the song's top edge is at or above the offset, and its bottom is below it
                        // This makes it "active" when it scrolls into the main content area, below the header.
                        if (rect.top <= FIXED_HEADER_OFFSET && rect.bottom > FIXED_HEADER_OFFSET) {
                    ...
                    // If no active song is found (e.g., in between songs, or at the start/end of a saga)
                    // then try to find an active saga.
                    if (!foundActiveSong) {
                         for (let i = sagas.length - 1; i >= 0; i--) {
                            const saga = sagas[i];
                            const rect = saga.getBoundingClientRect();
                            // Check if the saga's top edge is at or above the offset, and its bottom is below it
                            if (rect.top <= FIXED_HEADER_OFFSET && rect.bottom > FIXED_HEADER_OFFSET) {
                    ...
                    // Collapse all saga lists first, then expand the active one
            ...
            .then(([epicText, jsonData]) => {
                ...
                handleScroll(); // Call once on load to set initial active state and background
            })
            .catch(error => {
